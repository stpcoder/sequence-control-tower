...
const SAMPLE_MARKER = 'SCT_SAMPLE_LPDDR6_XIAOMI_V3'
const REFERENCE_MARKER = 'SCT_SAMPLE_LPDDR5_REFERENCE_V3'
const SAMPLE_FOLDER = 'lpddr6-xiaomi-v3'
...
const DEMO_NAMES = {
  ...
  screen2: demoFilename({ timestamp: '26-08-07-14-23-17', equipmentChannel: 8, evaluationNo: 2, temperatureC: 85, vdd: '1.295', conditions: ['SKEW-SS', 'LOT-A1', 'DIE03', 'DEFAULT', '9600MHZ', 'TM-VPERI', 'BASE', 'PAT-WR'], com: 74, sample: 'DHCST-90', evaluationStep: 'C', result: 'Fail' }),
  roomPass: demoFilename({ timestamp: '26-08-07-15-03-12', equipmentChannel: 8, evaluationNo: 3, temperatureC: 25, vdd: '1.295', conditions: ['SKEW-SS', 'LOT-A1', 'DIE04', 'DEFAULT', '9600MHZ', 'TM-VPERI', 'ROOM', 'PAT-WR'], com: 74, sample: 'DHCST-91', evaluationStep: 'C', result: 'Pass' }),
  trainingFail: demoFilename({ timestamp: '26-08-08-09-11-04', equipmentChannel: 6, evaluationNo: 1, temperatureC: -20, vdd: '1.275', conditions: ['SKEW-SF', 'LOT-B4', 'DIE07', 'DEFAULT', '8533MHZ', 'TM-BOOT', 'PAT-TRAIN'], com: 62, sample: 'DHCST-99', evaluationStep: 'C', result: 'TrainingFail' }),
  ...
  improvement2: demoFilename({ timestamp: '26-08-09-10-20-10', equipmentChannel: 8, evaluationNo: 2, temperatureC: 85, vdd: '1.315', conditions: ['SKEW-SS', 'LOT-A1', 'DIE03', 'VPERI-UP', '9600MHZ', 'TM-VPERI', 'IMPROVE', 'PAT-WR'], com: 74, sample: 'DHCST-90', evaluationStep: 'D', result: 'Pass' }),
  retentionHalt: demoFilename({ timestamp: '26-08-10-11-01-31', equipmentChannel: 10, evaluationNo: 1, temperatureC: 105, vdd: '1.295', conditions: ['SKEW-FF', 'LOT-C2', 'DIE09', 'DEFAULT', '9600MHZ', 'TM-RETENTION', 'PAT-MARCH'], com: 81, sample: 'DHCST-118', evaluationStep: 'C', result: 'SystemHalt' }),
  retentionPass: demoFilename({ timestamp: '26-08-10-11-21-31', equipmentChannel: 10, evaluationNo: 2, temperatureC: 105, vdd: '1.295', conditions: ['SKEW-FF', 'LOT-C2', 'DIE10', 'DEFAULT', '9600MHZ', 'TM-RETENTION', 'PAT-MARCH'], com: 81, sample: 'DHCST-119', evaluationStep: 'C', result: 'Pass' }),
...
const SAMPLE_EVALUATIONS = [
  { key: 'screen', folder: '01-vperi-screening', matches: (name: string) => !/_RT2_|_1\.315_|_TM-RETENTION_|_TrainingFail\.log$/i.test(name) },
...
export class SampleProjectService {
  ...
  async create(): Promise<ProjectLoadResult> {
    ...
    if (!project) project = await this.deps.projects.create({ name: 'LPDDR6 Xiaomi 16Gb VPERI 개발', description: `${SAMPLE_MARKER} · Agent Native 기능 확인용 샘플`, onboardingAnswers: { evaluationTarget: 'VPERI 불량 검출 조건과 개선 전압 확인', importantMetadata: 'sample, skew, lot, die, DQ, BL, channel, pattern, temperature, VDD, frequency, test mode, evaluation step', reuseRules: '@FAIL/training/reboot/halt 우선 판정; @PASS 확정' } })
