...
const SAMPLE_LOGS: Record<string, string> = {
  'LPDDR6_XIAOMI_16Gb_SKU-X6_LOT-A1_MAT-WAF12_SMP-01_T85_VDD1p295_F9600_TM-VPERI_PAT-WR-DQ9_BL16_CH0_FAIL.log': log('VPERI_SCREEN_01', 'TEMP=85C VDD=1.295V FREQ=9600MHz TM=VPERI PATTERN=WR DQ=9 BL=16 CH=0', [
  ...
  'LPDDR6_XIAOMI_16Gb_SKU-X6_LOT-A1_MAT-WAF12_SMP-02_T85_VDD1p295_F9600_TM-VPERI_PAT-WR-DQ9_BL16_CH0_FAIL.log': log('VPERI_SCREEN_02', 'TEMP=85C VDD=1.295V FREQ=9600MHz TM=VPERI PATTERN=WR DQ=9 BL=16 CH=0', [
  ...
  'LPDDR6_XIAOMI_16Gb_SKU-X6_LOT-A1_MAT-WAF12_SMP-03_T25_VDD1p295_F9600_TM-VPERI_PAT-WR_DQ9_BL16_CH0_PASS.log': log('VPERI_BASELINE_03', 'TEMP=25C VDD=1.295V FREQ=9600MHz TM=VPERI PATTERN=WR DQ=9 BL=16 CH=0', [
    '[00:00:10.030] stressapptest PASS', '[00:00:10.031] @PASS', '[00:00:10.032] TEST COMPLETE'
  ]),
  'LPDDR6_XIAOMI_16Gb_SKU-X6_LOT-B4_MAT-WAF27_SMP-11_T-20_VDD1p275_F8533_TM-BOOT_PAT-TRAIN_DQ20_BL32_CH1_TRAINFAIL.log': log('BOOT_MARGIN_11', 'TEMP=-20C VDD=1.275V FREQ=8533MHz TM=BOOT PATTERN=TRAIN DQ=20 BL=32 CH=1', [
  ...
  'LPDDR6_XIAOMI_16Gb_SKU-X6_LOT-A1_MAT-WAF12_SMP-01_T85_VDD1p315_F9600_TM-VPERI_PAT-WR_DQ9_BL16_CH0_PASS.log': log('VPERI_MARGIN_UP_01', 'TEMP=85C VDD=1.315V FREQ=9600MHz TM=VPERI PATTERN=WR DQ=9 BL=16 CH=0', [
    '[00:00:10.050] stressapptest PASS', '[00:00:10.051] @PASS', '[00:00:10.052] TEST COMPLETE'
  ]),
  'LPDDR6_XIAOMI_16Gb_SKU-X6_LOT-A1_MAT-WAF12_SMP-02_T85_VDD1p315_F9600_TM-VPERI_PAT-WR_DQ9_BL16_CH0_PASS.log': log('VPERI_MARGIN_UP_02', 'TEMP=85C VDD=1.315V FREQ=9600MHz TM=VPERI PATTERN=WR DQ=9 BL=16 CH=0', [
    '[00:00:10.060] stressapptest PASS', '[00:00:10.061] @PASS', '[00:00:10.062] TEST COMPLETE'
  ]),
  'LPDDR6_XIAOMI_16Gb_SKU-X6_LOT-C2_MAT-WAF31_SMP-18_T105_VDD1p295_F9600_TM-RETENTION_PAT-MARCH_DQ4_BL16_CH2_HALT.log': log('RETENTION_18', 'TEMP=105C VDD=1.295V FREQ=9600MHz TM=RETENTION PATTERN=MARCH DQ=4 BL=16 CH=2', [
    '[00:00:10.070] ERROR DQ4 timeout waiting for completion', '[00:00:10.071] CPU_HALT fatal exception'
  ]),
  'LPDDR6_XIAOMI_16Gb_SKU-X6_LOT-C2_MAT-WAF31_SMP-19_T105_VDD1p295_F9600_TM-RETENTION_PAT-MARCH_DQ4_BL16_CH2_PASS.log': log('RETENTION_19', 'TEMP=105C VDD=1.295V FREQ=9600MHz TM=RETENTION PATTERN=MARCH DQ=4 BL=16 CH=2', [
...
export class SampleProjectService {
  ...
  async create(): Promise<ProjectLoadResult> {
    ...
    connected = await this.deps.projects.save({
      ...
      failureHypotheses: [
        { id: 'sample-h-vperi-dq9', title: '고온 VPERI DQ9 집중', description: '85°C, VDD 1.295V에서 DQ9 fail이 반복됨. VPERI 동일 기인은 엔지니어 확인 전 가설.', origin: 'ai-proposed', evaluationNodeIds: ['sample-n-screen', 'sample-n-vdd-up'] },
      ...
      evaluationNodes: [
        { id: 'sample-n-screen', hypothesisId: 'sample-h-vperi-dq9', branchId: 'vperi-screen', name: 'VPERI 불량 가속 조건 확인', dimensions: { sku: 'X6-16Gb', lot: 'A1', material: 'WAF12', dq: 9, bl: 16, channel: 0, pattern: 'WR', frequencyMHz: 9600, temperatureC: 85, vdd: 1.295, testMode: 'VPERI' }, status: 'fail' },
        { id: 'sample-n-vdd-up', hypothesisId: 'sample-h-vperi-dq9', parentId: 'sample-n-screen', branchId: 'vperi-improvement', name: 'VDD 1.315V 개선 확인', dimensions: { sku: 'X6-16Gb', lot: 'A1', material: 'WAF12', dq: 9, bl: 16, channel: 0, pattern: 'WR', frequencyMHz: 9600, temperatureC: 85, vdd: 1.315, testMode: 'VPERI' }, status: 'pass' },
        { id: 'sample-n-retention', hypothesisId: 'sample-h-retention', branchId: 'retention', name: '고온 retention 재현', dimensions: { sku: 'X6-16Gb', lot: 'C2', material: 'WAF31', dq: 4, bl: 16, channel: 2, pattern: 'MARCH', frequencyMHz: 9600, temperatureC: 105, vdd: 1.295, testMode: 'RETENTION' }, status: 'inconclusive' }
      ],
      evidenceRecords: [
        { id: 'sample-e-screen-fail', evaluationNodeId: 'sample-n-screen', occurredAt: stamp, status: 'fail', result: '2/2 FAIL at DQ9', sourceIds: ids('T85_VDD1p295_F9600_TM-VPERI'), note: '결정 규칙 @FAIL; 85°C 분모 2, 실패 2', origin: 'engineer-confirmed' },
