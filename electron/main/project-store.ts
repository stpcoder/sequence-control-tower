...
function agentProposal(value: unknown): NonNullable<ProjectEvaluationNode['agentProposal']> {
  if (!value || typeof value !== 'object') throw new Error('Agent 초안 정보가 올바르지 않습니다.')
  const source = value as Record<string, unknown>
  const basis = normalizeNativeEvaluationBasis(source.basis)
  if (!basis) throw new Error('Agent 초안의 평가 기준이 올바르지 않습니다.')
  return { sessionId: id(source.sessionId, 'Agent sessionId'), proposalId: id(source.proposalId, 'Agent proposalId'), basis }
}
