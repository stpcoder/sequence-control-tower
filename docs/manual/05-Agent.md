...
로그 검색 결과의 `AI`, 결과 화면의 `Agent로 비교`, 결과 정리의 `Agent로 해석`을 누르면 현재 화면에서 선택한 범위로 새 대화가 시작됩니다. 분석에 필요한 평가 목적이 미확인인 경우 Agent가 먼저 한 가지를 묻고, 답변이 끝나면 선택한 검색 또는 표 분석을 자동으로 이어갑니다.
