...
- `평가 이력` 페이지는 Agent가 아닙니다. 확인 후 저장된 평가와 폴더 관계를 보는 프로젝트 기록 화면입니다. 이 화면의 `Agent로 분석`은 우측 패널의 평가 정리 Agent를 실행합니다.
...
2. 로그 화면 하단의 `Agent`를 누릅니다. 다른 화면에서는 `Agent에게 묻기`를 누릅니다.
3. `새 대화`를 누릅니다.
4. 질문을 입력합니다.
...
5. 다른 폴더에서는 SoC, 부팅 프로파일, Mode, Pattern이 맞을 때만 재사용 후보로 제시합니다. 검색어, 발생 개수, 순서를 현재 폴더에 맞게 수정한 뒤 미리 적용합니다.
...
로그 검색 결과의 `AI`, 결과 화면의 `Agent로 비교`, 결과 정리의 `Agent 분석`을 누르면 현재 화면에서 선택한 범위로 새 대화가 시작됩니다. 결과 정리에서는 현재 시각화, 가로·세로 항목, 계산 기준, 선택한 조건이 함께 전달됩니다. Agent는 화면 숫자를 그대로 복사하지 않고 로컬 분석 도구로 현재 평가 폴더를 다시 계산합니다. 분석에 필요한 평가 목적이 미확인인 경우 Agent가 먼저 한 가지를 묻고, 답변이 끝나면 선택한 검색 또는 분석 보기 검토를 이어갑니다.
