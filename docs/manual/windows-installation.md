...
이미 설치된 Sequence Control Tower가 감지되면 설치 프로그램이 확인 메시지를 표시합니다. 계속을 선택하면 electron-builder가 설치 전에 이전 버전의 앱 바이너리만 자동으로 제거한 뒤 새 버전을 설치합니다. 취소를 선택하면 설치를 진행하지 않습니다. 이 과정에서 안정적인 앱 ID를 사용하므로 사용자 데이터와 설정(`%APPDATA%\Sequence Control Tower\sequence-intelligence`)은 보존되며, 설치 프로그램이 사용자 데이터 폴더나 레지스트리 트리를 직접 삭제하지 않습니다.
...
설치형 업데이트에서 기존 설치가 감지되면 위 확인 메시지에서 계속을 선택하세요. electron-builder가 이전 앱 바이너리를 자동으로 교체하고 사용자 데이터는 업데이트 후에도 유지됩니다. 회사 백업 정책이 있으면 업데이트 전에 적용합니다.
