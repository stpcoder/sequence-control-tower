...
N × M 표는 같은 결과를 온도×VDD, Sample×Pattern, DQ×BL처럼 비교할 때 사용합니다. 미확인 후보가 있는 경우 해당 값은 확정 데이터와 구분됩니다.
...
각 평가에는 목적, 결과, Sample, Lot, Material, Die, SKU, 온도, VDD, 주파수, test mode, pattern, DQ, BL, Channel, Bank, Bank Group, SKEW를 기록할 수 있습니다.

평가 제목이나 설명을 Agent에게 적으면 Agent 대화로 전달해 조건 후보와 과거 사례를 먼저 확인할 수 있습니다. 저장 전에는 반드시 엔지니어가 결과와 관계를 확정합니다.
