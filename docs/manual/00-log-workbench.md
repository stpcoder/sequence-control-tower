...
Log Workbench는 여러 폴더의 `.log` 파일을 읽기 전용으로 등록하고, 검색 근거와 엔지니어 판정, 일괄 판정 결과를 한 화면에서 관리하는 데스크톱 도구입니다. 로그·원문·판정 3개 영역의 너비를 조절할 수 있으며 기본 로그 작업에는 LLM API를 호출하지 않습니다.

PASS, DIAG_FAIL, SYSTEM_HALT, 조건 불일치와 같은 실제 작업은 [SoC 실장 평가 로그 실무 예제](01-practical-workflows.md)를 참고합니다.

## 화면 구성

왼쪽 메뉴는 `로그`, `결과`, `결과 정리`, `설정`으로 구성됩니다.

## 화면 이름

| 화면 | 뜻 |
|---|---|
| 결과 | 필터·편집·내보내기를 하는 로그 결과 목록 |
| 결과 정리 | 현재 범위의 결과를 N × M 조건표와 집계 추세로 정리하는 화면 |
...
| 결과 | 메타데이터 후보 승인, 판정 검토, CSV·TSV 내보내기 |
| 결과 정리 | 결과 분포와 Sample·온도·Mode별 결과 확인 |
...
`결과`에는 `파일명`, `폴더`, `Sample`, `온도`, `Mode`, `Grid`, `진행 단계`, `결과`, `판정 검토`, `근거`가 표시됩니다. `진행 단계`는 플랫폼별 최종 펌웨어 단계, OS 도달, 실제 테스트 결과만 표시합니다. Qualcomm은 UEFI 계열, MediaTek은 LK/LK2 계열을 사용합니다. 부팅만 확인한 로그에는 테스트 단계가 표시되지 않습니다. 파일명에서 네 필드를 추출합니다. 값 하나는 `후보`로 검토하고, 값이 없거나 서로 다른 후보가 충돌하면 `미확인`으로 표시됩니다. 파일명에 값이 없으면 로그 본문을 검사합니다.
...
`결과 정리`의 `분석 표`에서 `로그 수`, `Fail 수`, `Evidence 수`를 선택합니다. `세로`와 `가로`에 `Sample`, `온도`, `Mode`, `Grid`, `결과`, `검토`, `폴더`, `Run`을 선택합니다. 축 값이 모두 미확인이면 축 변경 또는 값 검토 안내가 표시됩니다. 후보만 있으면 미승인 후보 기반 미리보기로 표시됩니다. 결과 셀을 누르면 아래 원본 로그 목록이 필터링됩니다.
