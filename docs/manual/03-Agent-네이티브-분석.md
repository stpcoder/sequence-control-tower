...
`사용자 질문` → `제품 프로젝트와 평가 폴더 고정` → `읽기 전용 도구 선택` → `로컬 수치 계산` → `필요한 로그 구간만 확인` → `LLM 해석` → `근거가 포함된 답변` → `엔지니어 확정`

Agent는 선택한 로그가 속한 폴더를 한 번의 평가 범위로 사용하고, 최대 100개 로그를 한 질문의 범위로 사용합니다. 여러 폴더가 연결된 프로젝트에서는 먼저 로그를 선택합니다. 전체 원문을 LLM에 보내지 않고 검색 위치와 최대 24줄 구간만 읽습니다.
...
- 한 로그에서 배운 Ctrl-F 순서를 같은 평가 폴더로 확장하고 다른 폴더와 분리
...
| `project_history_get` | 현재 평가 폴더에 연결된 불량 가설, 평가 노드, 근거 조회 |
| `similar_case_search` | 다른 프로젝트의 유사 평가 검색 |
| `search_history_get` | 현재 평가 폴더의 최근 Ctrl-F·정규식 행동 조회 |
...
Agent 첫 화면의 `결과와 평가 이력 정리`는 현재 평가 폴더의 파일명 조건을 먼저 읽고, marker 검색과 최대 24줄 근거 창을 제한적으로 사용합니다. 다음 항목을 제안합니다.
...
- 정성 해석: 실패가 집중된 조건, 비교 조건의 차이, 불확실성, 다음 확인 항목

결론을 바꾸는 정보가 부족하면 Agent가 한 가지를 먼저 질문합니다. 제안은 저장되지 않은 상태로 표시됩니다. `결과·이력 저장`을 누르면 근거 줄, 평가 폴더 ID, 정성 해석과 함께 결과가 저장되고, 같은 source ID가 평가 이력에 연결됩니다. 저장된 해설은 `AI 작성 · 엔지니어 확인`으로 표시됩니다. `다시 분석`을 누르면 제안을 폐기하고 다시 확인합니다.
...
대화 세션에는 평가 폴더 ID가 함께 저장됩니다. 같은 제품 프로젝트 안에서도 다른 폴더의 대화와 검색 절차는 현재 평가에 자동 적용되지 않습니다. 제품·고객이 실제로 다른 로그는 같은 폴더에 섞지 말고 별도 제품 프로젝트로 등록해야 합니다.
