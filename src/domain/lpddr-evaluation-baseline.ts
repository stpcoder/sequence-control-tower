...
- 주파수는 일반적으로 파일명에 없습니다. 파일명에 547, 5333, Enable5333Only처럼 명확한 표기가 있으면 후보로 보존하고, 없으면 console의 clk.sh·setddrclk 입력이나 관련 출력에서 설정값을 확인합니다. 로그에도 값이 없으면 기본 주파수를 추정하지 않고 미확인으로 둡니다.
...
- 조건 경향은 “85°C 2/2 FAIL, 25°C 0/1 FAIL이므로 고온에서 불량이 더 잘 재현되는 경향”처럼 분자와 분모로 표현합니다. 온도·VDD·주파수 조건만으로 “고온 기인” 같은 원인 표현을 사용하지 않습니다. FAIL 로그 사이에서 BK3, DQ9, BL16처럼 반복되는 공통점은 Fail address event 수와 포함 로그 수를 함께 적습니다.
- 원인 가능성은 Test Mode 변경 전후처럼 비교 조건이 통제되고 기존 Fail signature의 변화가 확인될 때 좁힙니다. Sample·Die·Sequence도 함께 달라졌다면 재현 가능성이 높은 조건을 먼저 말하고, 함께 달라진 항목은 다음 비교 변수로 남깁니다.
...
export function explicitLpddrConditions(text: string): LpddrConditionDimensions {
  ...
  const frequencyMHz = decimal(captured(text, /(?:^|[^A-Z0-9])(?:FREQ(?:UENCY)?|DDRCLK|CLK)[=:_ -]+(\d{3,5})(?:\s*(?:MHZ|MT))?/i)
    ...
    ?? captured(text, /(?:^|[^A-Z0-9])CLK\.SH\s+(?:-F\s+)?(\d{3,5})(?=\s|$)/i))
