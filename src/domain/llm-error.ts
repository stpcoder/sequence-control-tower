...
export function llmErrorCode(error: unknown): string {
  ...
  return /LLM_HTTP_\d{3}/.exec(raw)?.[0]
    ?? /LLM_(?:REQUEST_TIMEOUT|REQUEST_FAILED|UNAVAILABLE|TPM_REQUEST_TOO_LARGE|INVALID_JSON_RESPONSE|INVALID_TOOL_CALL|EMPTY_RESPONSE|REASONING_ONLY_RESPONSE|RESPONSE_TOO_LARGE|VERTEX_AUTH_UNAVAILABLE)/.exec(raw)?.[0]
    ?? ''
}
...
export function llmFailureDisplay(error: unknown): string | null {
  ...
  if (code === 'LLM_INVALID_TOOL_CALL') return 'LLM이 올바르지 않은 도구 호출을 반환했습니다. 다시 시도해 주세요.'
