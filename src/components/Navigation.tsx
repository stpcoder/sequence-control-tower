...
export function Navigation({ active, onChange, onAgentOpen }: { active: AppPage; onChange: (page: AppPage) => void; onAgentOpen: () => void }) {
  return (
    <aside className="navigation">
      ...
      <button className="agent-health" onClick={onAgentOpen}>
      ...
      <button className={active === 'settings' ? 'nav-item active' : 'nav-item'} onClick={() => onChange('settings')}>
        <Settings size={18} />
        <span>
      ...
      <div className="build-version">LOCAL-FIRST · POC 0.1</div>
    </aside>
  )
}
