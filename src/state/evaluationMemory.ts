...
export function projectSnapshotToEvaluationMemory(project: ProjectSnapshot): EvaluationMemory {
  ...
  return {
    project: { id: project.id, name: project.name, product: context.product, sku: context.sku, program: context.program, phase: context.phase, customer: context.customer, targetDevice: context.targetDevice, densityGb: context.densityGb, nominalVoltage: context.nominalVoltage },
...
export function evaluationMemoryToProjectSave(memory: EvaluationMemory): EvaluationMemorySave {
  return {
    lpddrDevelopmentContext: { product: memory.project.product, sku: memory.project.sku, program: memory.project.program, phase: memory.project.phase, customer: memory.project.customer, targetDevice: memory.project.targetDevice, densityGb: memory.project.densityGb, nominalVoltage: memory.project.nominalVoltage },
