...
export function EvaluationMemoryView({ memory, availableLogs, onChange, onOpenLog, onNotify, onAskAgent }: EvaluationMemoryViewProps) {
  ...
  const askAgent = () => {
    ...
    onAskAgent(`다음 평가 내용을 분석해 평가 이력으로 정리해줘. 온도, VDD, SKEW, 자재, Die, Sample, SoC, 부팅 단계, Pattern, DQ, BL, Channel과 Pass/Fail 근거를 확인하고 불확실한 핵심 항목만 질문해줘.\n\n${request}`)
  }
  ...
  return <div className="evaluation-memory-view">
    ...
    <div className="evaluation-memory-view__layout">
      <main><EvaluationLineage memory={memory} selectedNodeId={selectedNodeId} onSelectNode={(node) => setSelectedNodeId(node.id)} />
        {trends.length ? <section className="evaluation-memory-view__signals"><div className="evaluation-memory-view__section-label">반복 불량</div>{trends.map((trend) => <div className="evaluation-memory-view__trend" key={`${trend.dimension}-${trend.value}`}><b>{trendDimensionLabel(trend.dimension)} = {trend.value}</b><span>{trend.failureCount}/{trend.evidenceCount} FAIL · {Math.round(trend.failureRate * 100)}%</span><em>{trend.origin === 'engineer-confirmed' ? '확정' : '제안'}</em></div>)}</section> : null}
      </main>
      <aside className="evaluation-memory-view__editor">
        ...
        <details className="evaluation-memory-view__manual">
          ...
          <div className="evaluation-memory-view__section-label">평가</div><input value={evaluation.name} onChange={(event) => setEvaluation({ ...evaluation, name: event.target.value })} placeholder="평가 이름" /><div className="evaluation-memory-view__pair"><select value={evaluation.hypothesisId} onChange={(event) => setEvaluation({ ...evaluation, hypothesisId: event.target.value })}><option value="">가설 없음</option>{memory.hypotheses.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><select value={evaluation.parentId} onChange={(event) => setEvaluation({ ...evaluation, parentId: event.target.value })}><option value="">이전 평가 없음</option>{memory.nodes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div><div className="evaluation-memory-view__pair"><input value={evaluation.branchId} onChange={(event) => setEvaluation({ ...evaluation, branchId: event.target.value })} placeholder="분석 흐름" /><select value={evaluation.status} onChange={(event) => setEvaluation({ ...evaluation, status: event.target.value as EvaluationStatus })}><option value="inconclusive">판정 보류</option><option value="pass">통과</option><option value="fail">불량</option><option value="running">진행 중</option></select></div>
