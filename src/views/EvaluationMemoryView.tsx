...
export function EvaluationMemoryView({ memory, availableLogs, onChange, onOpenLog, onNotify }: EvaluationMemoryViewProps) {
  ...
  return <div className="evaluation-memory-view">
    <header className="evaluation-memory-view__header"><span>평가 {memory.nodes.length} · 근거 {memory.evidence.length}</span><div className="evaluation-memory-view__actions"><button onClick={() => downloadCsv(evaluationMemoryCsv(memory))}>CSV</button><button onClick={() => void copyContext()}>AI 맥락</button></div></header>
    ...
    <div className="evaluation-memory-view__layout">
      <main><EvaluationLineage memory={memory} selectedNodeId={selectedNodeId} onSelectNode={(node) => setSelectedNodeId(node.id)} />
        {trends.length ? <section className="evaluation-memory-view__signals"><div className="evaluation-memory-view__section-label">반복 불량</div>{trends.map((trend) => <div className="evaluation-memory-view__trend" key={`${trend.dimension}-${trend.value}`}><b>{trend.dimension} = {trend.value}</b><span>{trend.failureCount}/{trend.evidenceCount} FAIL · {Math.round(trend.failureRate * 100)}%</span><em>{trend.origin === 'engineer-confirmed' ? '확정' : '제안'}</em></div>)}</section> : null}
      </main>
      <aside className="evaluation-memory-view__editor">
        <div className="evaluation-memory-view__section-label">가설 추가</div><input disabled={saving} value={hypothesis.title} onChange={(event) => setHypothesis({ ...hypothesis, title: event.target.value })} placeholder="가설 이름" /><textarea disabled={saving} value={hypothesis.description} onChange={(event) => setHypothesis({ ...hypothesis, description: event.target.value })} placeholder="근거 메모" /><label className="evaluation-memory-view__origin"><select disabled={saving} value={hypothesis.origin} onChange={(event) => setHypothesis({ ...hypothesis, origin: event.target.value as AssessmentOrigin })}><option value="ai-proposed">AI 제안</option><option value="engineer-confirmed">엔지니어 확인</option></select><button disabled={saving} onClick={() => void addHypothesis()}>{saving ? '저장 중…' : '추가'}</button></label>
        <div className="evaluation-memory-view__section-label">평가 추가</div><input value={evaluation.name} onChange={(event) => setEvaluation({ ...evaluation, name: event.target.value })} placeholder="평가 이름" /><div className="evaluation-memory-view__pair"><select value={evaluation.hypothesisId} onChange={(event) => setEvaluation({ ...evaluation, hypothesisId: event.target.value })}><option value="">가설 없음</option>{memory.hypotheses.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><select value={evaluation.parentId} onChange={(event) => setEvaluation({ ...evaluation, parentId: event.target.value })}><option value="">이전 평가 없음</option>{memory.nodes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div><div className="evaluation-memory-view__pair"><input value={evaluation.branchId} onChange={(event) => setEvaluation({ ...evaluation, branchId: event.target.value })} placeholder="분석 흐름" /><select value={evaluation.status} onChange={(event) => setEvaluation({ ...evaluation, status: event.target.value as EvaluationStatus })}><option value="inconclusive">판정 보류</option><option value="pass">통과</option><option value="fail">불량</option><option value="running">진행 중</option></select></div>
