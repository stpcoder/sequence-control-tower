...
import { Check, Download, FileText, Folder, Sparkles } from 'lucide-react'
...
export function evaluationFolderFlow(memory: EvaluationMemory, groups: readonly EvaluationFolderGroup[]): EvaluationFolderFlowItem[] {
  ...
  const items = groups.map((group) => {
    ...
    const parentGroupId = [...group.nodes].reverse().map((item) => item.parentId ? groupByNodeId.get(item.parentId) : undefined).find((parentId) => parentId && parentId !== group.id)
  ...
  return <div className="data-view evaluation-memory-view">
    ...
    <div className="evaluation-memory-view__workspace">
      ...
      <aside className="evaluation-memory-view__review">
        <header><div><strong>선택 평가 분석</strong><span>{selectedGroup?.label ?? '평가를 선택하세요'}</span></div></header>
        {selectedGroup ? <>
          <button className="evaluation-memory-view__agent" disabled={!onAnalyzeEvaluation || !selectedGroup.logs.length} onClick={() => onAnalyzeEvaluation?.({ evaluationScopeId: selectedGroup.id, title: selectedGroup.label, sourceIds: selectedGroup.logs.map((log) => log.id), openId: selectedGroup.logs[0]?.openId })}><Sparkles size={14} />Agent로 분석</button>
