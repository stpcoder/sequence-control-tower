...
import type { AssessmentOrigin, EvaluationDimensions, EvaluationMemory, EvaluationNode, EvaluationStatus, EvidenceRecord, FailureHypothesis, ProductProject } from '../domain/evaluation-memory'
...
export function addEvaluationWithEvidence(memory: EvaluationMemory, input: { name: string; hypothesisId?: string; parentId?: string; branchId?: string; status: EvaluationStatus; dimensions: EvaluationDimensions; logIds: readonly string[]; origin: AssessmentOrigin }): EvaluationMemory {
  const node: EvaluationNode = { id: id('eval'), projectId: memory.project.id, name: input.name, hypothesisId: input.hypothesisId || undefined, parentId: input.parentId || undefined, branchId: input.branchId || undefined, status: input.status, dimensions: input.dimensions }
...
export function buildEvaluationContextMarkdown(memory: EvaluationMemory): string {
  ...
  lines.push(...memory.nodes.map((node) => `- ${node.id} ${node.name} [${node.branchId ?? 'main'}] ${node.status ?? 'inconclusive'}${node.parentId ? ` ← ${node.parentId}` : ''}${node.hypothesisId ? ` · hypothesis ${node.hypothesisId}` : ''}`))
...
export function EvaluationMemoryView({ memory, availableLogs, onChange, onOpenLog, onNotify }: EvaluationMemoryViewProps) {
  ...
  const [evaluation, setEvaluation] = useState({ name: '', hypothesisId: '', parentId: '', branchId: '', status: 'inconclusive' as EvaluationStatus, origin: 'ai-proposed' as AssessmentOrigin, dimensions: emptyDimensions(), logIds: [] as string[] })
  ...
  const addEvaluation = async () => {
    ...
    if (await save(next)) { setSelectedNodeId(added?.id); setEvaluation({ name: '', hypothesisId: '', parentId: '', branchId: '', status: 'inconclusive', origin: 'ai-proposed', dimensions: emptyDimensions(), logIds: [] }) }
  }
  ...
  return <div className="evaluation-memory-view">
    ...
    <div className="evaluation-memory-view__layout">
      <main><EvaluationLineage memory={memory} selectedNodeId={selectedNodeId} onSelectNode={(node) => setSelectedNodeId(node.id)} />
        <section className="evaluation-memory-view__signals"><div className="evaluation-memory-view__section-label">반복 불량 경향</div>{trends.length ? trends.map((trend) => <div className="evaluation-memory-view__trend" key={`${trend.dimension}-${trend.value}`}><b>{trend.dimension} = {trend.value}</b><span>실패율 {trend.failureCount}/{trend.evidenceCount} · {Math.round(trend.failureRate * 100)}%</span><em>{trend.origin === 'engineer-confirmed' ? '엔지니어 확인' : 'AI 제안'}</em></div>) : <p>아직 반복되는 불량 경향이 없습니다.</p>}</section>
      </main>
      <aside className="evaluation-memory-view__editor">
        <div className="evaluation-memory-view__section-label">불량 가설</div><input disabled={saving} value={hypothesis.title} onChange={(event) => setHypothesis({ ...hypothesis, title: event.target.value })} placeholder="가설 이름" /><textarea disabled={saving} value={hypothesis.description} onChange={(event) => setHypothesis({ ...hypothesis, description: event.target.value })} placeholder="근거 메모 (선택)" /><label className="evaluation-memory-view__origin"><select disabled={saving} value={hypothesis.origin} onChange={(event) => setHypothesis({ ...hypothesis, origin: event.target.value as AssessmentOrigin })}><option value="ai-proposed">AI 제안</option><option value="engineer-confirmed">엔지니어 확인</option></select><button disabled={saving} onClick={() => void addHypothesis()}>{saving ? '저장 중…' : '추가'}</button></label>
        <div className="evaluation-memory-view__section-label">새 평가</div><input value={evaluation.name} onChange={(event) => setEvaluation({ ...evaluation, name: event.target.value })} placeholder="평가 이름" /><div className="evaluation-memory-view__pair"><select value={evaluation.hypothesisId} onChange={(event) => setEvaluation({ ...evaluation, hypothesisId: event.target.value })}><option value="">가설 없음</option>{memory.hypotheses.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><select value={evaluation.parentId} onChange={(event) => setEvaluation({ ...evaluation, parentId: event.target.value })}><option value="">부모 없음</option>{memory.nodes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div><div className="evaluation-memory-view__pair"><input value={evaluation.branchId} onChange={(event) => setEvaluation({ ...evaluation, branchId: event.target.value })} placeholder="브랜치 / 이슈" /><select value={evaluation.status} onChange={(event) => setEvaluation({ ...evaluation, status: event.target.value as EvaluationStatus })}><option value="inconclusive">판정 보류</option><option value="pass">통과</option><option value="fail">불량</option><option value="running">진행 중</option></select></div>
        ...
        {selectedNode && <div className="evaluation-memory-view__selected">선택됨: <b>{selectedNode.name}</b>{selectedLogIds.length ? <div className="evaluation-memory-view__selected-logs">{selectedLogIds.map((logId) => { const log = availableLogs.find((item) => item.id === logId); return <button key={logId} type="button" onClick={() => onOpenLog(openIdForEvidenceLog(logId, availableLogs))}>{log?.name ?? logId} 열기</button> })}</div> : <span> · 연결된 로그가 없습니다.</span>}</div>}
      </aside>
    </div>
  </div>
}
