...
import { ArrowDown, ArrowUp, ChevronDown, Clipboard, Download, FilterX, Search, SlidersHorizontal, X } from 'lucide-react'
...
export function ResultsView({ records, onOpenFile, onApproveMetadata, onEditMetadata, onNotify }: ResultsViewProps) {
  ...
  const filtered = useMemo(() => sortLogRecords(filterLogRecords(records, { query, result, review, folder }).filter((row) => {
    ...
    return true
  }), sortKey, sortDirection), [query, records, result, review, folder, preset, sortDirection, sortKey])
  ...
  const hasFilters = Boolean(query || result !== 'all' || review !== 'all' || folder !== 'all' || preset)
          ...
          <tbody>
            {visible.map((row) => (
              <tr key={row.id} tabIndex={0} onClick={() => onOpenFile(row.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onOpenFile(row.id) } }} aria-label={`${row.fileName} 로그 열기`}>
                ...
                {(['sample', 'temperature', 'mode', 'grid'] as const).map((field) => <td key={field} onClick={(event) => { event.stopPropagation(); if (editingCell?.rowId !== row.id || editingCell.field !== field) beginEdit(row, field) }}>
                  {editingCell?.rowId === row.id && editingCell.field === field && onEditMetadata ? <form className="metadata-inline-editor" onSubmit={(event) => { event.preventDefault(); void saveEdit() }} onClick={(event) => event.stopPropagation()}>
                    <input autoFocus value={editingValue} onChange={(event) => setEditingValue(event.target.value)} aria-label={`${field} 값 직접 입력`} placeholder="미확인" />
                    <button type="submit" disabled={!editingValue.trim()}>저장</button><button type="button" onClick={() => setEditingCell(null)} aria-label="편집 취소"><X size={13} /></button>
                  </form> : candidateLabel(row[field], field === 'temperature' ? '°C' : '', onApproveMetadata ? (value) => void onApproveMetadata(row, field, value) : undefined)}
                </td>)}
