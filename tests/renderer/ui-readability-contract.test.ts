...
describe('UI readability contract', () => {
  ...
  it('uses one full-width evidence row and hides verbose paused-agent copy', async () => {
    ...
    expect(panel).not.toContain("disabled={busy}>조건별 불량 경향")
    expect(panel).not.toContain("disabled={busy}>과거 사례와 다음 평가")
    ...
    expect(panel).not.toContain('aria-label="현재 작업 단계"')
    expect(panel).not.toContain('AGENT_EVALUATION_STAGES.map')
    expect(panel).toContain('nativeSession.question.choices.map')
  ...
  it('keeps review and rule surfaces concise', async () => {
    ...
    expect(results).not.toContain("{ key: 'evidenceCount', label: '판정 근거 줄' }")
    expect(results).not.toContain('title="PASS·FAIL·Halt·Reboot 등 현재 판정에 사용된 로그 줄 수"')
    expect(workbench).not.toContain('검색 {searchHistory[activeFile.id]?.length ?? 0} · 근거')
    expect(workbench).not.toContain('그러면 <b>')
    expect(workbench).not.toContain('<strong>검색 절차 저장</strong>')
    expect(workbench).toContain('Explicitly applying the rule also confirms the same Ctrl-F procedure')
    expect(workbench).not.toContain('<strong>평가 하네스</strong>')
    expect(workbench).toContain("<strong>{editingRecipeId ? '규칙 편집' : '규칙 추가'}</strong><span>판정에 사용할 검색을 선택하세요</span>")
    expect(workbench).toContain('<Braces size={14} /> 규칙 관리')
    expect(workbench).toContain('<SlidersHorizontal size={14} />규칙 편집')
    expect(workbench).not.toContain('<section className="pattern-review"')
    ...
    expect(workbench).not.toContain("ruleEditorUi.showOpenButton && !recipeSaved")
    expect(workbench).toContain("'현재 폴더에 적용'")
    expect(workbench).toContain('{batchExceptionFiles.map((file) =>')
    expect(workbench).toContain('{ruleEditorUi.showEditor && draft ? (')
    expect(workbench).not.toContain('workflow-confirmation recipe-suggestion')
  })
  ...
  it('removes duplicate page copy and uses direct grid directions', async () => {
    ...
    expect(patterns).toContain('aria-label="결과 정리 표. 좌우로 스크롤할 수 있습니다."')
    expect(patterns).toContain("const PIVOT_ROW_AXIS_WIDTH = 132")
    expect(project).toContain('{busy ? <LoaderCircle className="wb-spin" size={15} /> : null}프로젝트 만들기')
    ...
    expect(project).not.toContain('무엇을 추출·결정할까요?')
    expect(project).not.toContain('결과에 표시할 항목')
    expect(project).toContain('PROJECT_INIT_ITEMS = [\'Sample\', \'Skew\', \'온도\', \'VDD\'')
    expect(project).toContain('프로젝트 만들기')
  ...
  it('keeps result stages concise and history in a folder-scoped qualitative workspace', async () => {
    ...
    expect(memory).toContain('Agent로 정리')
    expect(memory).toContain('평가 목적')
    expect(memory).toContain('평가 결과')
    expect(memory).toContain('결과 해석')
    expect(memory).toContain('불량 경향')
    expect(memory).toContain('다음 평가')
