...
describe('UI readability contract', () => {
  ...
  it('keeps review and rule surfaces concise', async () => {
    ...
    expect(workbench.match(/<strong>판정 규칙<\/strong><span>검색 조건 선택<\/span>/g)?.length).toBe(1)
    expect(workbench).toContain("<strong>{editingRecipeId ? '규칙 수정' : '규칙 추가'}</strong><span>판정에 사용할 검색을 선택하세요</span>")
