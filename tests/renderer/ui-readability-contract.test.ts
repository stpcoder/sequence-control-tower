...
describe('UI readability contract', () => {
  ...
  it('keeps review and rule surfaces concise', async () => {
    ...
    expect(workbench).toContain('<strong>분석 규칙</strong><span>판정에 사용할 검색을 선택하세요</span>')
  })
  ...
  it('keeps result stages concise and history in a folder-scoped qualitative workspace', async () => {
    ...
    expect(memory).toContain('프로젝트 평가 흐름')
    expect(memory).toContain('조건별 경향')
    expect(memory).toContain('이 평가의 로그')
    expect(memory).toContain('선택 평가 분석')
    ...
    expect(memoryStyles).toContain('grid-template-columns: minmax(420px,1fr) 340px')
