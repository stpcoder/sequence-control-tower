...
const project = { id: 'project-1', name: '검증 프로젝트' } as ProjectSnapshot
const file = { id: 'source-1', name: 'sample.log', artifactId: 'artifact-1' } as WorkbenchFile
...
describe('mini agent domain safety', () => {
  it('starts with the selected artifact and maps only a known result for confirmation', () => {
    expect(buildAgentDecisionInput(project, file, snapshot, 'PASS')).toMatchObject({
      projectId: 'project-1', expectedRevision: 7, source: { sourceId: 'source-1', artifactId: 'artifact-1' }, result: 'PASS',
    })
