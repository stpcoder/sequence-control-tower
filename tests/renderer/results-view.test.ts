...
describe('renderer results export', () => {
  it('downloads one BOM followed by the 16-column metadata/result default header and row', async () => {
    ...
    expect(lines[0].slice(1).split(',')).toHaveLength(16)
    expect(lines[1].split(',')).toHaveLength(16)
