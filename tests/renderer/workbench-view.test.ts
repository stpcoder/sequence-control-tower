...
describe('Log Workbench UI data hardening', () => {
  ...
  it('wires the review UI to the existing analysis lifecycle and keeps it review-only', () => {
    ...
    expect(workbenchSource).toContain('selected && recipeDetailsOpen')
    expect(workbenchSource).not.toContain('recipe-order-modal')
    expect(workbenchCss).not.toContain('.recipe-order-modal')
    ...
    expect(workbenchSource).not.toContain('저장하고 현재 폴더 적용')
